      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {premium && !demoMode && paymentStep === "pay" ? (
            ...
            <div className="mx-4 mt-2 mb-4 p-4 border rounded-lg"><div className="flex justify-between mb-2"><span className="text-sm font-semibold">Stake Amount (USDT)</span></div><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2">$</span><input type="text" inputMode="decimal" value={stakeAmount} onChange={e => { if (/^\d*\.?\d*$/.test(e.target.value)) setStakeAmount(e.target.value); }} placeholder={bot.min_stake.toFixed(2)} className="w-full h-12 pl-7 pr-16 rounded-lg bg-secondary text-lg font-semibold" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs">USDT</span></div><div className="flex gap-2 mt-3">{ [bot.min_stake, 100, 500].map(amt => (<button key={amt} onClick={() => setStakeAmount(amt.toString())} className="flex-1 text-xs py-1.5 rounded-md bg-primary/10 text-primary">${amt}</button>)) }<button onClick={() => setStakeAmount(effectiveBalance.toFixed(2))} className="flex-1 text-xs py-1.5 rounded-md bg-primary/10 text-primary">MAX</button></div><div className="flex justify-between mt-2"><span className="text-[11px] text-muted-foreground">{demoMode ? "Demo " : ""}Balance: ${effectiveBalance.toFixed(2)}</span></div>{amount > 0 && amount < bot.min_stake && <p className="text-[11px] text-loss mt-1">Min stake ${bot.min_stake}</p>}{amount > effectiveBalance && !premium && <p className="text-[11px] text-loss mt-1">Insufficient balance. <Link to="/deposit" className="text-primary">Deposit →</Link></p>}</div>
          )}
        </div>
