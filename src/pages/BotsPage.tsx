...
const BotsPage = () => {
  ...
  const [botChatEnabled, setBotChatEnabled] = useState(true); // bots can send messages
  ...
  // --- Chat: Add system message when a new trade occurs (bots talking) ---
  useEffect(() => {
    ...
    if (!chatMessages.some(m => m.id === lastTrade.id)) {
      ...
      setChatMessages(prev => [message, ...prev].slice(0, 150));
      // Optional: emit a sound or badge
    }
  ...
  useEffect(() => {
    if (chatOpen && chatMessages.length === 0) {
      setChatMessages([
        {
          id: "welcome",
          text: "👋 Welcome to **Bot Chat**! I'll notify you of every trade. Try:\n• `/status` – your bot summary\n• `/help` – all commands\n• `/bots` – list available bots\n• `/price BTC` – get current price",
          timestamp: new Date(),
          type: "system",
        },
      ]);
  ...
  // Simulated trade generator (unchanged logic, but also sends chat message)
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      myBots.forEach(async (bot: any) => {
        ...
        // Optional: send a chat message from the bot itself (if you want more verbose)
        if (botChatEnabled && Math.random() < 0.3) { // only 30% of trades to avoid spam
          const chatMsg: ChatMessage = {
            id: `bot-${Date.now()}-${Math.random()}`,
            text: `🤖 ${bot.name} just executed a ${side === "buy" ? "LONG" : "SHORT"} on ${symbol}/USDT. Profit: $${pnl.toFixed(2)} 🚀`,
            timestamp: new Date(),
            type: "bot",
          };
          setChatMessages(prev => [chatMsg, ...prev].slice(0, 150));
        }
      });
    }, 3000);
  ...
  const unstakeBot = useMutation({
    mutationFn: async (bot: any) => {
      ...
      setChatMessages(prev => [
        {
          id: Date.now().toString(),
          text: `🛑 **Unstaked** ${bot.name}. Returned $${returnAmount.toFixed(2)} USDT to your wallet.`,
          timestamp: new Date(),
          type: "system",
        },
        ...prev,
  ...
  // Chat command handler (enhanced)
  const handleSendMessage = () => {
    const msg = chatInput.trim();
    if (!msg) return;
    setChatMessages(prev => [
      { id: Date.now().toString(), text: msg, timestamp: new Date(), type: "user" },
      ...prev,
    ]);
    setChatInput("");
    ...
    if (lowerMsg === "/status") {
      ...
      if (runningBots.length === 0) {
        addBotMessage("📊 You have no running bots. Copy a bot from the list to start.");
      } else {
      ...
      addBotMessage("🤖 **Available commands**\n• `/status` – Your bot summary\n• `/bots` – List available bots\n• `/price <coin>` – Get current price (e.g. `/price BTC`)\n• `/clear` – Clear chat\n• `/toggle` – Enable/disable bot trade messages");
    } else if (lowerMsg === "/bots") {
      const botNames = filteredBots.slice(0, 10).map(b => `• ${b.name} (${b.tier || "free"})`).join("\n");
      addBotMessage(`**Top bots available:**\n${botNames}\n\nUse /bot <name> for details.`);
    } else if (lowerMsg === "/clear") {
      setChatMessages([]);
      addBotMessage("Chat cleared.");
    } else if (lowerMsg === "/toggle") {
      setBotChatEnabled(prev => !prev);
      addBotMessage(`Bot trade messages ${!botChatEnabled ? "enabled ✅" : "disabled ❌"}.`);
    } else if (lowerMsg.startsWith("/price")) {
      const parts = msg.split(" ");
      const symbolInput = parts[1]?.toUpperCase();
      if (!symbolInput) {
        addBotMessage("Usage: `/price BTC` or `/price ETH`");
      } else {
        const found = prices.find(p => p.symbol.toLowerCase() === symbolInput.toLowerCase());
        if (found) {
          addBotMessage(`💰 **${found.symbol.toUpperCase()}/USDT** price: $${found.current_price.toLocaleString()} (24h: ${found.price_change_percentage_24h?.toFixed(2)}%)`);
        } else {
          addBotMessage(`Coin "${symbolInput}" not found. Try /price BTC`);
        }
      }
    } else if (lowerMsg.startsWith("/bot ")) {
      const botName = msg.slice(5).toLowerCase();
      const bot = filteredBots.find(b => b.name.toLowerCase().includes(botName));
      if (bot) {
        addBotMessage(`**${bot.name}** | Tier: ${bot.tier || "free"} | Strategy: ${STRATEGY_LABELS[bot.strategy] || bot.strategy}\nProfit: $${bot.total_profit.toLocaleString()} | Min stake: $${bot.min_stake}\nRun: /stake ${bot.id} to start.`);
      } else {
        addBotMessage(`Bot "${botName}" not found. Use /bots to see list.`);
      }
    } else if (lowerMsg.startsWith("/stake")) {
      addBotMessage("Please select a bot from the list and click 'Copy' to stake.");
    } else {
      // AI-like generic response
      addBotMessage(`I'm here to help! Type \`/help\` for commands. You said: "${msg}"`);
    }
  };

  const addBotMessage = (text: string) => {
    setChatMessages(prev => [
      { id: Date.now().toString(), text, timestamp: new Date(), type: "bot" },
      ...prev,
    ]);
  };
    ...
    return (
      <div className="flex flex-col h-full">
          ...
          {premium && !demoMode && paymentStep === "pay" ? (
            <div className="mx-3 sm:mx-4 mt-4 mb-4">
              <div className="bg-card border border-primary/30 rounded-xl p-4">
                ...
                <button onClick={() => { navigator.clipboard.writeText(depositAddress); toast.success("Address copied!"); }} className="w-full text-xs py-2 rounded-lg bg-primary/10 text-primary hover:bg-primary/20 transition-colors mb-3">Copy Address</button>
                <p className="text-[10px] text-muted-foreground text-center">After sending, click "I've Paid" below. Our system will verify the transaction.</p>
              </div>
            </div>
          ) : (
            <div className="mx-3 sm:mx-4 mt-2 mb-4">
              <div className="bg-card border border-border rounded-lg p-4">
                ...
                <div className="relative">
                  ...
                  <input type="text" inputMode="decimal" pattern="[0-9]*\.?[0-9]*" value={stakeAmount} onChange={e => { const val = e.target.value; if (val === "" || /^\d*\.?\d*$/.test(val)) setStakeAmount(val); }} placeholder={bot.min_stake.toFixed(2)} className="w-full h-12 pl-7 pr-16 rounded-lg bg-secondary border border-border text-lg font-semibold text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">USDT</span>
                </div>
                <div className="flex gap-2 mt-3">
                  {[bot.min_stake, 100, 500].map(amt => (<button key={amt} onClick={() => setStakeAmount(amt.toString())} className="flex-1 text-xs py-1.5 rounded-md bg-primary/10 text-primary hover:bg-primary/20 transition-colors">${amt}</button>))}
                  <button onClick={() => setStakeAmount(effectiveBalance.toFixed(2))} className="flex-1 text-xs py-1.5 rounded-md bg-primary/10 text-primary hover:bg-primary/20 transition-colors">MAX</button>
                </div>
                <div className="flex items-center justify-between mt-2">
                  <span className="text-[11px] text-muted-foreground">{demoMode ? "Demo " : ""}Balance: <span className="text-foreground font-medium">${effectiveBalance.toFixed(2)}</span> USDT</span>
                </div>
                {amount > 0 && amount < bot.min_stake && <p className="text-[11px] text-loss mt-1">Minimum stake is ${bot.min_stake.toFixed(2)} USDT</p>}
                {amount > effectiveBalance && !premium && <p className="text-[11px] text-loss mt-1">Insufficient balance. <Link to="/deposit" className="text-primary hover:underline">Deposit →</Link></p>}
              </div>
            </div>
          )}
        </div>
        <div className="p-3 sm:p-4 border-t border-border">
          {premium && !demoMode && paymentStep === "pay" ? (
            <Button className="w-full h-12 text-sm font-bold bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg" onClick={() => { toast.success("Payment verification started. Bot will activate once confirmed."); setPaymentStep("monitoring"); setTimeout(() => { stakeBot.mutate({ bot, amount: Number(stakeAmount) || bot.min_stake }); setPaymentStep("info"); }, 3000); }}><CreditCard className="h-4 w-4 mr-2" /> I've Paid — Verify & Start Bot</Button>
          ) : premium && !demoMode && paymentStep === "monitoring" ? (
            <Button className="w-full h-12 text-sm font-bold rounded-lg" disabled><RefreshCw className="h-4 w-4 mr-2 animate-spin" /> Verifying payment...</Button>
          ) : premium && !demoMode ? (
            <Button className="w-full h-12 text-sm font-bold bg-profit hover:bg-profit/90 text-white rounded-lg disabled:opacity-50" disabled={amount < bot.min_stake} onClick={() => setPaymentStep("pay")}>{amount >= bot.min_stake ? `Pay $${amount.toFixed(2)} to Activate` : "Enter Stake Amount"}</Button>
          ) : (
            <Button className="w-full h-12 text-sm font-bold bg-profit hover:bg-profit/90 text-white rounded-lg disabled:opacity-50" disabled={!canStake || stakeBot.isPending} onClick={() => stakeBot.mutate({ bot, amount })}>{stakeBot.isPending ? "Processing..." : canStake ? "Sign Terms & Start Bot" : amount > effectiveBalance ? "Insufficient Balance" : "Enter Stake Amount"}</Button>
          )}
          <p className="text-[10px] text-muted-foreground text-center mt-2">By clicking, you agree to the bot's terms. Staked funds will be managed by the bot strategy.</p>
        </div>
      </div>
    );
  };
  ...
  // ------------------------------------------------------------
  // RENDER: Responsive Layout (Desktop + Mobile)
  // ------------------------------------------------------------
  return (
    <DashboardLayout>
      <TradePopup />
      ...
      <div className="flex flex-col h-[calc(100vh-3.5rem)] md:h-[calc(100vh-0px)] overflow-hidden">
        ...
        <div className="flex items-center gap-4 px-4 py-2 border-b border-border bg-card text-sm overflow-x-auto whitespace-nowrap">
          ...
          <span className="ml-auto text-xs text-muted-foreground">{getSymbol(selectedChartPair)}/USDT</span>
        </div>

        {/* Main content: two different layouts for desktop vs mobile */}
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
              <div className="px-4 py-2 flex items-center gap-4 border-b border-border">
                <div className="relative" ref={dropdownRef}>
                  <button onClick={() => setPairDropdownOpen(!pairDropdownOpen)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-secondary transition-colors">
                    {selectedPairPrice && <img src={selectedPairPrice.image} alt="" className="w-5 h-5 rounded-full" />}
                    <span className="text-sm font-bold text-foreground">{getSymbol(selectedChartPair)}/USDT</span>
                  ...
                  {pairDropdownOpen && (
                    <div className="absolute top-full left-0 mt-1 w-72 bg-card border border-border rounded-xl shadow-2xl z-50 overflow-hidden">
                      <div className="p-2"><input type="text" value={pairSearch} onChange={e => setPairSearch(e.target.value)} placeholder="Search pairs..." className="w-full h-8 rounded-lg bg-secondary border border-border px-3 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary" autoFocus /></div>
                      <div className="max-h-64 overflow-y-auto">
                        {TRADEABLE.filter(id => { const p = prices.find(pr => pr.id === id); return p && (p.name.toLowerCase().includes(pairSearch.toLowerCase()) || p.symbol.toLowerCase().includes(pairSearch.toLowerCase())); }).map(id => {
                          ...
                          if (!p) return null;
                          return (
                            <button key={id} onClick={() => { setSelectedChartPair(id); setPairDropdownOpen(false); setPairSearch(""); }} className={`w-full flex items-center justify-between px-3 py-2 text-xs hover:bg-secondary/80 transition-colors ${selectedChartPair === id ? "bg-primary/10" : ""}`}>
                              <div className="flex items-center gap-2"><img src={p.image} alt="" className="w-5 h-5 rounded-full" /><span className="font-medium text-foreground">{p.symbol.toUpperCase()}/USDT</span></div>
                              <div className="text-right"><span className="text-foreground font-medium">${p.current_price.toLocaleString()}</span><span className={`ml-2 ${p.price_change_percentage_24h >= 0 ? "text-profit" : "text-loss"}`}>{p.price_change_percentage_24h >= 0 ? "+" : ""}{p.price_change_percentage_24h.toFixed(2)}%</span></div>
                            </button>
                          );
                        })}
                      </div>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-lg font-bold text-foreground tabular-nums">${currentPrice.toLocaleString()}</span>
              ...
              <div className="flex-1 min-h-0 bg-background p-4"><div ref={chartRef} className="w-full h-full min-h-[300px]" /></div>
              <div className="border-t border-border bg-card flex-shrink-0">
                <div className="flex gap-6 px-4 overflow-x-auto">
                  {(["running", "history", "pnl"] as const).map(t => (<button key={t} className={`py-3 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${bottomTab === t ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`} onClick={() => setBottomTab(t)}>{t === "running" ? "Running" : t === "history" ? "History" : "PNL Analysis"}</button>))}
                </div>
                <div className="p-4 overflow-auto max-h-[280px]">
                  {bottomTab === "running" && (
                    myBots.filter(b => b.status === "running").length === 0 ? <div className="text-center py-8 text-muted-foreground text-sm"><Bot className="h-8 w-8 mx-auto mb-2 opacity-30" /><p>No bots running. Copy a bot to get started.</p></div> :
                    <div className="space-y-2">
                      {myBots.filter(b => b.status === "running").map((bot: any) => {
                        const staked = Number(bot.config?.staked_amount || 0);
                        const profit = Number(bot.total_profit || 0);
                        return (
                          <div key={bot.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-secondary/50 rounded-lg border border-border gap-2 cursor-pointer hover:border-primary/30 transition-colors" onClick={() => setViewingRunningBot(bot)}>
                            <div><p className="text-sm font-medium text-foreground">{bot.name}</p><p className="text-[11px] text-muted-foreground">{getSymbol(bot.crypto_id)}/USDT • Staked: ${staked.toFixed(2)}</p></div>
                            <div className="flex items-center justify-between sm:justify-end gap-3"><div className="text-right"><p className={`text-sm font-bold ${profit >= 0 ? "text-profit" : "text-loss"}`}>{profit >= 0 ? "+" : ""}${profit.toFixed(2)}</p><p className="text-[10px] text-muted-foreground">{bot.total_trades} trades</p></div><Button variant="outline" size="sm" className="text-[10px] h-7 text-loss border-loss/30 hover:bg-loss/10" onClick={(e) => { e.stopPropagation(); if (confirm(`Stop bot and withdraw $${(staked + profit).toFixed(2)} USDT?`)) unstakeBot.mutate(bot); }} disabled={unstakeBot.isPending}>{unstakeBot.isPending ? "..." : "Unstake"}</Button></div>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  {bottomTab === "history" && (
                    userTrades.length === 0 ? <p className="text-center text-muted-foreground text-sm py-8">No trade history yet.</p> :
                    <div className="overflow-x-auto"><table className="w-full text-xs min-w-[500px]"><thead><tr className="text-muted-foreground border-b border-border"><th className="text-left py-2">Pair</th><th className="text-left py-2">Position</th><th className="text-right py-2">Price</th><th className="text-right py-2">Amount</th><th className="text-right py-2">PNL</th><th className="text-right py-2">Time</th></tr></thead><tbody>{userTrades.slice(0,20).map((t:any)=> (<tr key={t.id} className="border-b border-border/30"><td className="py-1.5 text-foreground">{getSymbol(t.crypto_id)}/USDT</td><td className={`py-1.5 font-medium ${t.side==="buy"?"text-profit":"text-loss"}`}>{t.side==="buy"?"Long":"Short"}</td><td className="py-1.5 text-right text-foreground">${Number(t.price).toLocaleString()}</td><td className="py-1.5 text-right text-foreground">{Number(t.amount).toFixed(4)}</td><td className={`py-1.5 text-right ${(t.pnl||0)>=0?"text-profit":"text-loss"}`}>${(t.pnl||0).toFixed(2)}</td><td className="py-1.5 text-right text-muted-foreground">{new Date(t.created_at).toLocaleTimeString()}</td></tr>))}</tbody></table></div>
                  )}
                  {bottomTab === "pnl" && (
                    pnlChartData.length === 0 ? <div className="text-center py-8 text-muted-foreground text-sm"><BarChart3 className="h-8 w-8 mx-auto mb-2 opacity-30" /><p>No PNL data available yet. Start a bot to see your profit over time.</p></div> :
                    <div className="h-64"><ResponsiveContainer width="100%" height="100%"><LineChart data={pnlChartData}><CartesianGrid strokeDasharray="3 3" stroke="#333" /><XAxis dataKey="date" tick={{ fill: "#888" }} /><YAxis tick={{ fill: "#888" }} /><Tooltip contentStyle={{ backgroundColor: "#1f1f1f", border: "1px solid #333" }} labelStyle={{ color: "#fff" }} /><Line type="monotone" dataKey="profit" stroke="#22c55e" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></div>
                  )}
                </div>
              </div>
            </div>
            {/* Right sidebar (desktop) */}
            <div className="w-[320px] lg:w-[360px] border-l border-border bg-card flex flex-col overflow-hidden shrink-0">
              {viewingRunningBot ? <BotAnalyticsView bot={viewingRunningBot} onBack={() => setViewingRunningBot(null)} onUnstake={(bot) => { unstakeBot.mutate(bot); setViewingRunningBot(null); }} unstaking={unstakeBot.isPending} /> :
               ...
               <div className="flex flex-col h-full overflow-y-auto">
                 <div className="flex items-center gap-3 px-4 pt-4 pb-2"><button className={`text-sm font-medium px-3 py-1.5 rounded-lg transition-colors ${mainTab === "popular" ? "bg-secondary text-foreground" : "text-muted-foreground hover:text-foreground"}`} onClick={() => setMainTab("popular")}>Popular</button><button className={`text-sm font-medium px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1 ${mainTab === "ai" ? "bg-primary/20 text-primary border border-primary/30" : "text-muted-foreground hover:text-foreground"}`} onClick={() => setMainTab("ai")}><Zap className="h-3.5 w-3.5" /> AI</button><button className="ml-auto text-muted-foreground hover:text-foreground" onClick={() => runBotsNow.mutate()} disabled={runBotsNow.isPending}><RefreshCw className={`h-4 w-4 ${runBotsNow.isPending ? "animate-spin" : ""}`} /></button></div>
                 <div className="px-4 pb-2"><div className="relative"><Search className="h-3.5 w-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><input type="text" placeholder="Search bots..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full h-8 pl-9 pr-3 rounded-lg bg-secondary border border-border text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary" /></div></div>
                 <div className="flex gap-1.5 px-4 pb-2 flex-wrap overflow-x-auto">{TIER_FILTERS.map(t => (<button key={t} className={`text-[10px] font-bold px-2.5 py-1 rounded-full border transition-colors whitespace-nowrap ${tierFilter === t ? t === "All Tiers" ? "bg-primary/20 text-primary border-primary/30" : t === "Free" ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/30" : t === "Pro" ? "bg-blue-500/20 text-blue-400 border-blue-500/30" : t === "Elite" ? "bg-purple-500/20 text-purple-400 border-purple-500/30" : "bg-amber-500/20 text-amber-400 border-amber-500/30" : "border-border text-muted-foreground hover:text-foreground"}`} onClick={() => setTierFilter(t)}>{t}</button>))}</div>
                 <div className="flex gap-1.5 px-4 pb-3 flex-wrap overflow-x-auto">{STRATEGY_FILTERS.map(s => (<button key={s} className={`text-[10px] font-medium px-2 py-0.5 rounded border transition-colors whitespace-nowrap ${strategyFilter === s ? "bg-primary/20 text-primary border-primary/30" : "border-border text-muted-foreground hover:text-foreground"}`} onClick={() => setStrategyFilter(s)}>{s}</button>))}</div>
                 {mainTab === "ai" && <div className="px-4 pb-3"><p className="text-[11px] text-muted-foreground flex items-center gap-1"><Zap className="h-3 w-3 text-primary" /><span className="font-semibold text-primary">AI-Powered Bots</span> — neural signal trading across all tiers</p></div>}
                 <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-3">{filteredBots.length === 0 ? <p className="text-center text-muted-foreground text-sm py-8">No bots found.</p> : filteredBots.map((bot: any) => <BotCard key={bot.id} bot={bot} />)}</div>
                 <div className="px-4 py-3 border-t border-border"><p className="text-[10px] text-muted-foreground flex items-center gap-1"><Zap className="h-3 w-3 text-primary" /> Daily win % refreshes every 4 hours. Results credited to wallet.</p></div>
               </div>}
            </div>
          </div>

          {/* MOBILE (< md) - single column with bot list always visible */}
          <div className="flex flex-col h-full md:hidden">
            {/* Chart area (smaller) */}
            <div className="flex-shrink-0 px-4 py-2 flex items-center gap-4 border-b border-border">
              <div className="relative" ref={dropdownRef}>
                <button onClick={() => setPairDropdownOpen(!pairDropdownOpen)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-secondary transition-colors">
                  {selectedPairPrice && <img src={selectedPairPrice.image} alt="" className="w-5 h-5 rounded-full" />}
                  <span className="text-sm font-bold text-foreground">{getSymbol(selectedChartPair)}/USDT</span>
                ...
                {pairDropdownOpen && (/* same dropdown content as desktop */)}
              </div>
              <div className="flex items-center gap-2">
                <span className="text-lg font-bold text-foreground tabular-nums">${currentPrice.toLocaleString()}</span>
                {selectedPairPrice && <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${selectedPairPrice.price_change_percentage_24h >= 0 ? "bg-profit/10 text-profit" : "bg-loss/10 text-loss"}`}>{selectedPairPrice.price_change_percentage_24h >= 0 ? "+" : ""}{selectedPairPrice.price_change_percentage_24h.toFixed(2)}%</span>}
            ...
            <div className="border-t border-border bg-card flex-shrink-0">
              <div className="flex gap-6 px-4 overflow-x-auto">
                {(["running", "history", "pnl"] as const).map(t => (<button key={t} className={`py-3 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${bottomTab === t ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`} onClick={() => setBottomTab(t)}>{t === "running" ? "Running" : t === "history" ? "History" : "PNL"}</button>))}
              </div>
              <div className="p-4 overflow-auto max-h-[180px]">
                {bottomTab === "running" && (myBots.filter(b => b.status === "running").length === 0 ? <p className="text-center text-muted-foreground text-sm">No bots running.</p> : myBots.filter(b => b.status === "running").map((bot: any) => (<div key={bot.id} className="flex justify-between items-center p-2 bg-secondary/50 rounded-lg mb-2"><div><p className="text-sm font-medium">{bot.name}</p><p className="text-[10px] text-muted-foreground">Staked: ${(bot.config?.staked_amount||0).toFixed(2)}</p></div><div><p className={`text-sm font-bold ${bot.total_profit>=0?"text-profit":"text-loss"}`}>${bot.total_profit?.toFixed(2)}</p><Button size="sm" variant="outline" className="text-[10px] h-6 mt-1" onClick={() => setViewingRunningBot(bot)}>Details</Button></div></div>)))}
                {bottomTab === "history" && (userTrades.length === 0 ? <p className="text-center text-muted-foreground text-sm">No trades yet.</p> : <div className="space-y-1">{userTrades.slice(0,5).map(t => (<div key={t.id} className="text-xs flex justify-between border-b border-border py-1"><span>{getSymbol(t.crypto_id)}</span><span className={t.side==="buy"?"text-profit":"text-loss"}>{t.side}</span><span>${Number(t.price).toFixed(2)}</span><span className={t.pnl>=0?"text-profit":"text-loss"}>${t.pnl?.toFixed(2)}</span></div>))}</div>)}
                {bottomTab === "pnl" && (pnlChartData.length === 0 ? <p className="text-center text-muted-foreground text-sm">No data</p> : <div className="h-40"><ResponsiveContainer><LineChart data={pnlChartData}><Line type="monotone" dataKey="profit" stroke="#22c55e" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></div>)}
              </div>
            </div>

            {/* Bot list (always visible on mobile) */}
            <div className="flex-1 overflow-hidden border-t border-border bg-card">
              {viewingRunningBot ? (
                ...
                <div className="flex flex-col h-full overflow-y-auto">
                  <div className="flex items-center gap-3 px-4 pt-4 pb-2"><button className={`text-sm font-medium px-3 py-1.5 rounded-lg ${mainTab === "popular" ? "bg-secondary text-foreground" : "text-muted-foreground"}`} onClick={() => setMainTab("popular")}>Popular</button><button className={`text-sm font-medium px-3 py-1.5 rounded-lg flex items-center gap-1 ${mainTab === "ai" ? "bg-primary/20 text-primary" : "text-muted-foreground"}`} onClick={() => setMainTab("ai")}><Zap className="h-3.5 w-3.5" /> AI</button><button className="ml-auto" onClick={() => runBotsNow.mutate()}><RefreshCw className="h-4 w-4" /></button></div>
                  <div className="px-4 pb-2"><div className="relative"><Search className="h-3.5 w-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><input type="text" placeholder="Search bots..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full h-8 pl-9 pr-3 rounded-lg bg-secondary text-xs" /></div></div>
                  <div className="flex gap-1.5 px-4 pb-2 flex-wrap">{TIER_FILTERS.map(t => (<button key={t} className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${tierFilter === t ? "bg-primary/20 text-primary border-primary/30" : "border-border text-muted-foreground"}`} onClick={() => setTierFilter(t)}>{t}</button>))}</div>
                  <div className="flex gap-1.5 px-4 pb-3 flex-wrap">{STRATEGY_FILTERS.map(s => (<button key={s} className={`text-[10px] px-2 py-0.5 rounded border ${strategyFilter === s ? "bg-primary/20 text-primary border-primary/30" : "border-border text-muted-foreground"}`} onClick={() => setStrategyFilter(s)}>{s}</button>))}</div>
        ...
        {/* Floating Chat */}
        {!chatOpen && (
          <button onClick={() => setChatOpen(true)} className="fixed bottom-6 right-6 bg-primary text-primary-foreground rounded-full p-3 shadow-lg hover:bg-primary/90 transition-all z-50">
        ...
        {chatOpen && (
          <div className="fixed bottom-6 right-6 w-80 h-96 bg-card border border-border rounded-xl shadow-2xl flex flex-col z-50">
            <div className="flex items-center justify-between px-4 py-2 border-b border-border">
              <span className="text-sm font-semibold text-foreground flex items-center gap-2"><Bot className="h-4 w-4" /> Bot Chat</span>
            ...
            <div className="p-2 border-t border-border flex gap-2">
              <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSendMessage()} placeholder="Type /help for commands..." className="flex-1 h-8 px-2 rounded-md bg-secondary text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary" />
